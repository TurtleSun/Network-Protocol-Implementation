...
/* Helper function to make packets, handles adding seqnum and checksum */
...
{	
  ...
  if(isDuplicate(packet)){
    // Duplicate ACK - retransmit first unACKed packet in window
    printf("A: Duplicate ACK for packet %d detected\n", packet.acknum);
    total_packets_retransmitted++;
    tolayer3(A, buffer[buffer_start].packet);
    stoptimer(A);
    starttimer(A, RXMT_TIMEOUT);
  }else{
...
if (packet.seqnum == expectedSeqNum) {
    ...
    while (!is_buffer_empty(B) && buffer_B[buffer_start_B].seqnum == expectedSeqNum && WINDOW_SIZE != 1) {
